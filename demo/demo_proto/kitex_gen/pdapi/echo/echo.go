...
func echoHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(pdapi.Request)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(pdapi.Echo).Echo(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *EchoArgs:
		success, err := handler.(pdapi.Echo).Echo(ctx, s.Req)
		if err != nil {
			return err
		}
...
var EchoArgs_Req_DEFAULT *pdapi.Request

func (p *EchoArgs) GetReq() *pdapi.Request {
	if !p.IsSetReq() {
...
type EchoResult struct {
	Success *pdapi.Response
}

var EchoResult_Success_DEFAULT *pdapi.Response

func (p *EchoResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(pdapi.Response)
	}
...
func (p *EchoResult) GetSuccess() *pdapi.Response {
	if !p.IsSetSuccess() {
...
func (p *kClient) Echo(ctx context.Context, Req *pdapi.Request) (r *pdapi.Response, err error) {
	var _args EchoArgs
	_args.Req = Req
